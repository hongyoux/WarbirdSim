...
glm::vec3 unitNormal(...)
bool std::isnanVec3(...)    // updated for g++-7
bool std::isnanVec4(...)    // updated for g++-7
bool std::isnanMat4(...)    // updated for g++-7
...
// An extension of the C  bool std::isnan(number).
// std::isnan(...) used for compliance with g++-7 compiler C11 compiler
...
bool isNanMat4(const glm::mat4 &m) {
  ...
  for(int i = 0; i < 4; i++)
      for(int j = 0; j < 4; j++)
        if(std::isnan(m[i][j])) result = true;
   return result;
   }
